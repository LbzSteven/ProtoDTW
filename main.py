...
def main():
    from utils.settings import base_architecture, experiment_run, prototype_shape, prototype_activation_function, \
        add_on_layers_type
    ...
    prototype_size = parser_else_settings(args.size_prototypes, prototype_shape[2])
    # set up prototype_shape
    prototype_num_total = parser_else_settings(args.num_prototypes * num_classes, prototype_shape[0])
    prototype_shape = (prototype_num_total, prototype_dim, prototype_size) \
    ...
    for epoch in range(num_train_epochs):
        ...
        if epoch < num_warm_epochs:
            ...
            save.save_model_w_condition(model=ppnet, model_dir=model_dir, model_name=str(epoch) + 'warm',
                                        accu=accu_test,
            ...
            if projection:
                push.push_prototypes(
                    ...
                    root_dir_for_saving_prototypes=root_dir_for_saving_prototypes,
                    # if not None, prototypes will be saved here  ts_dir
            ...
            result_test = tnt.test(model=ppnet_multi, dataloader=test_loader,
                                   class_specific=class_specific, log=log)
            accu_test = result_test[0]
            save.save_model_w_condition(model=ppnet, model_dir=model_dir, model_name=str(epoch) + 'epoch',
                                        accu=accu_test,
                                        target_accu=target_accu, log=log)
        ...
        if log_dir is not None:
            ...
            writer.close()
    save.save_model(model=ppnet, model_dir=model_dir, model_name='finish_epoch', log=log)

    log_result, log_result_close = create_logger(log_filename='logs/results.log', display=False)
    log_result(f'model_dir: {model_dir},{experiment_run}, Final Train ACC:{accu:.2f} Test ACC:{accu_test:.2f}')
    log_result_close()
    log(f'Final Train ACC:{accu:.2f} Test ACC:{accu_test:.2f}')
    print(model_dir, experiment_run, accu, accu_test)
    logclose()
if __name__ == '__main__':
    ...
    parser.add_argument('--size_prototypes', type=int, default=3, help='size of prototypes')
