...
def save_model_w_condition(model, model_dir, model_name, accu, target_accu, log=print):
    ...
    '''
    if accu > target_accu:
        log('\tabove {0:.2f}%'.format(target_accu))
...
def save_model(model, model_dir, model_name, log=print):
    # torch.save(obj=model.state_dict(), f=os.path.join(model_dir, (model_name + '{0:.4f}.pth').format(accu)))
    torch.save(obj=model, f=os.path.join(model_dir, (model_name + 'final.pth')))
